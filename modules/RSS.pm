...
sub loader {
  ...
  $help->add_help("rss", "Channel", "<add|del|list|set|sync> [#chan] [feed name] [url] [Interval(optional)]", "RSS module interface.", 0, sub {
    my ($nick, $host, $text) = @_;
    ...
    $bot->say($nick, "  \x02add\x02 #chan <feed name> <url> <Interval(optional)> - Adds a feed for a channel.");
